import { normalPrice } from '../../functions/normalPrice.js';
        ...
        if (i < quanL) {
          ...
          <div class="type__element" data-el-id="${i}">
            <div class="type__upper">
              ...
              <span class="type__all link-hover"><a href="category.html">Все товары&nbsp;&rsaquo;</a></span>
            </div>
            <div class="type__lower">
              <ul class="type__list-card list-reset" data-list-id="${i}">
              </ul>
            </div>
          </div>`;
          ...
          for (let k = 0; k < prod.length; k++) {
            ...
            const currentColorsCode = Object.values(el.colors);
            const currentColorsNames = Object.keys(el.colors);
            ...
            list.innerHTML += `
              <li class="type__item" data-code="${el.chars.Артикул}">
                ...
                <div class="type__colors">
                  <span class="type__color" style="background-color: ${currentColorsCode[0]}" aria-label="Один из цветов товара ${currentColorsNames[0]}" title="${currentColorsNames[0]}"></span>
                  <span class="type__color" style="background-color: ${currentColorsCode[1]}" aria-label="Один из цветов товара ${currentColorsNames[1]}" title="${currentColorsNames[1]}"></span>
                  <span class="type__color" style="background-color: ${currentColorsCode[2]}" aria-label="Один из цветов товара ${currentColorsNames[2]}" title="${currentColorsNames[2]}"></span>
                </div>
              </li>`;
          }
        }
